...
export class PrevendeurService {
  ...
  getDrilldown(annee_mois: string, code_fdv?: string | null, canal?: string | null, nom_distributeur?: string | null) {
    const key = `${annee_mois}|${code_fdv ?? ''}|${canal ?? ''}|${nom_distributeur ?? ''}`;
