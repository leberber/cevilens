...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  private loadInitial() {
    ...
    this.svc.getDrilldown(guess, this.selectedFdv, this.canal, this.selectedDistributeur).subscribe({
      next: d => {
  ...
  load(keepSelection = false) {
    ...
    this.svc.getDrilldown(this.selectedPeriode, this.selectedFdv, this.canal, this.selectedDistributeur).subscribe({
      next: d => {
        this.applyData(d);
  ...
  setDistributeur(distributeur: string | null) {
    if (this.selectedDistributeur === distributeur) return;
    this.selectedDistributeur = distributeur;
    this.selectedFdv = null;
    this.load(true); // preserve famille selection
  }

  onDistributeurChange() {
    this.selectedFdv = null;
    this.showDistMenu = false;
    this.load(true);
  }
  ...
  toggleDistMenu() {
    this.showDistMenu = !this.showDistMenu;
  }

  selectDistributeur(d: string) {
    this.selectedDistributeur = d;
    this.onDistributeurChange();
  }
